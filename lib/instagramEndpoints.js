...
async function fetchRecentReelMedia({ token, userId, limit = 100 }) {
  ...
  for (let page = 0; page < maxPages && reels.length < limit; page++) {
    const params = {
      fields:
        `id,caption,media_type,media_product_type,permalink,timestamp,thumbnail_url,insights.metric(${metricsJoined})`,
      limit: pageLimit,
    }
    if (after) params.after = after
    ...
    if (!ok || body.error) {
      const msg = body.error
        ? body.error.message || JSON.stringify(body.error)
        : 'Erreur liste médias Instagram'
      throw new Error(msg)
    }

    for (const item of body.data || []) {
      if (item.media_product_type === 'REELS' || item.media_type === 'VIDEO') {
        const { insights, ...media } = item
        reels.push({
          media,
          insights: parseMediaInsights(insights),
        })
      }
    }
...
export async function fetchTopReels({ token, userId, limit = 5, scanLimit = 100 }) {
  ...
  const rows = await Promise.all(
    reelItems.map(async ({ media, insights }) => {
      if (typeof insights?.views === 'number') {
        return {
          ok: true,
  ...
  return rows
    .filter((row) => typeof row.insights?.views === 'number')
    .sort((a, b) => b.insights.views - a.insights.views)
    .slice(0, limit)
}
