 ...
 */
function formatScaledPart(x, locale = 'fr-FR') {
  ...
  if (frac < 1e-9) {
    return int.toLocaleString(locale, { maximumFractionDigits: 0 })
  }
  return floored.toLocaleString(locale, {
 ...
 */
export function formatCompactMetric(value, locale = 'fr-FR') {
  ...
  if (abs >= 1_000_000) {
    ...
    return `${sign}${formatScaledPart(scaled, locale)}M`
  }
  if (abs >= 1_000) {
    const scaled = Math.floor((abs / 1_000) * 100) / 100
    return `${sign}${formatScaledPart(scaled, locale)}K`
  }

  const small = Math.floor(abs * 100) / 100
  return (
    sign +
    small.toLocaleString(locale, {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    })
  )
 ...
 */
export function formatPercent2(value, locale = 'fr-FR') {
  const n = Number(value)
  if (!Number.isFinite(n)) return '—'
